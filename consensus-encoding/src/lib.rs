...
pub trait Encode: for<'a> EncodeTc<'a> {
    ...
    #[cfg(feature = "hashes")]
    fn hash_consensus_encoded<H: hashes::Hash>(&self) -> H {
        self.encoder().hash()
    }
}
    ...
    ($($hash_type:ty => $vis:vis $decoder:ident;)*) => {
        $crate::push_decode::mapped_decoder! {
            $(
                ...
                $vis struct $decoder($crate::push_decode::decoders::ByteArrayDecoder<{<$hash_type>::LEN}>) using $hash_type => <$hash_type as $crate::hashes::Hash>::from_byte_array;
            )*
        }
