...
/// Re-export of the `bitcoin-io` crate.
pub extern crate io;

use core::{fmt, mem, u32};
#[cfg(not(feature = "std"))]
use alloc::{string::String, vec::Vec, boxed::Box, borrow::Cow, rc, vec};
#[cfg(feature = "std")]
use std::{string::String, vec::Vec, boxed::Box, borrow::Cow, rc, sync, vec};
...
/// Implements `Encodable` and `Decodable` for `Vec<$type>`.
#[macro_export]
