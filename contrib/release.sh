...
main () {
    for crate in "internals" "hashes" "dash"; do
...
publish_dry_run() {
    ...
    if [ "$crate" == "hashes" ]; then
        cargo publish -p "dashcore_hashes" --dry-run
    elif [ "$crate" == "internals" ]; then
        cargo publish -p "dashcore-private" --dry-run
    elif [ "$crate" == "dashcore" ]; then
        cargo publish -p "dashcore" --dry-run
    fi
}
