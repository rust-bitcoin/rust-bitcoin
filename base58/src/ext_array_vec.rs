...
use internals::slice::SliceExt as _;
...
impl<const N: usize> ArrayBuf<N> {
    ...
    fn as_slice(&self) -> &[MaybeUninit<u8>] {
        ...
        unsafe {
            core::slice::from_raw_parts(ptr.cast::<MaybeUninit<u8>>(), core::mem::size_of::<Self>())
        }
    }
...
impl<const N: usize> ExtendedArrayVec<N> {
    ...
    /// Tries to split this vector into a payload and checksum arrays.
    ///
    /// Returns `None` if the length of the vector is not `N + 4`.
    pub(crate) fn try_into_arrays(self) -> Option<([u8; N], [u8; 4])> {
        let (payload, checksum) = self.as_slice().split_last_chunk::<4>()?;
        let payload = <[u8; N]>::try_from(payload).ok()?;
