...
/// during decode.
pub fn decode_from_slice<T>(bytes: &[u8]) -> Result<T, DecodeError<T>>
where
    T: Decodable,
{
    let mut remaining = bytes;
    let data = decode_from_slice_unbounded(&mut remaining).map_err(DecodeError::<T>::Parse)?;

    if remaining.is_empty() {
        Ok(data)
    } else {
        Err(DecodeError::<T>::Unconsumed)
    }
}
...
pub enum DecodeError<T: Decodable> {
    /// Provided slice failed to correctly decode as a type.
    Parse(<T::Decoder as Decoder>::Error),
    /// Bytes remained unconsumed after completing decoding.
    #[non_exhaustive] Unconsumed,
}

impl<T: Decodable> From<Infallible> for DecodeError<T> {
    fn from(never: Infallible) -> Self { match never {} }
}

impl<T: Decodable> fmt::Display for DecodeError<T>
where
    <<T as Decodable>::Decoder as Decoder>::Error: fmt::Display
{
...
impl<T: Decodable> fmt::Debug for DecodeError<T>
where
    <<T as Decodable>::Decoder as Decoder>::Error: fmt::Display
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
#[cfg(feature = "std")]
impl<T: Decodable> std::error::Error for DecodeError<T>
where
    <<T as Decodable>::Decoder as Decoder>::Error: std::error::Error + 'static
{
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
