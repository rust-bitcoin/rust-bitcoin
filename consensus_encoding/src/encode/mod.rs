...
/// Attempting to call any method after [`advance`](Self::advance) returned `EncoderStatus::Finished` or calling
...
pub trait Encoder {
    ...
    /// # Important
    ///
    /// After `EncoderStatus::Finished` was returned the encoder is in unspecified state. Calling any of its methods
...
pub trait ExactSizeEncoder: Encoder {
    ...
    ///
    /// **Important**: returns an unspecified value if [`Encoder::advance`] has returned `EncoderStatus::Finished`.
    fn len(&self) -> usize;
