...
impl CompactSizeEncoder {
    /// Constructs a new `CompactSizeEncoder` from a `usize` length prefix value.
    ///
    ...
    /// Constructs a new `CompactSizeEncoder` from a `u64` value.
    ///
    /// Use this when encoding an arbitrary integer (e.g. service flags) rather than
    /// a length prefix. Unlike [`Self::new`], this constructor preserves all 64 bits
    /// on every platform.
