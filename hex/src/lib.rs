...
#![cfg_attr(all(not(test), not(feature = "std"), not(doctest)), no_std)]
...
impl<T: fmt::LowerHex> ToHex for T {
    fn to_hex(&self) -> String { format!("{:x}", self) }
}
...
impl HexWriter {
    ...
    pub fn new(capacity: usize) -> Self { HexWriter(String::with_capacity(capacity)) }

    /// Returns the resulting hex string.
    pub fn result(self) -> String { self.0 }
}
...
mod benches {
    ...
    fn bench_to_hex(bh: &mut Bencher) {
        let s = b"arbitrary content 32 byte string";
        bh.iter(|| {
            black_box(s.to_hex());
        })
    }
