...
use bitcoin::{script, ScriptBuf, ScriptPubkey};
...
fn main() {
    ...
    // Choose the script marker based on your context if known e.g., `RedeemScript`, `TapScript` etc.
    assert!(encode::deserialize_hex::<ScriptBuf<ScriptPubkey>>(&hex_lower_hex_trait).is_err());
    // And so does `from_hex_prefixed`.
    assert!(ScriptBuf::<ScriptPubkey>::from_hex_prefixed(&hex_lower_hex_trait).is_err());
    // But we provide an explicit constructor that does not.
    assert_eq!(
        ScriptBuf::<ScriptPubkey>::from_hex_no_length_prefix(&hex_lower_hex_trait).unwrap(),
        script_code
    );
    ...
    let decoded = encode::deserialize_hex::<ScriptBuf<ScriptPubkey>>(&hex_inherent).unwrap();
    assert_eq!(decoded, script_code);
    ...
    let decoded: ScriptBuf<ScriptPubkey> = encode::deserialize_hex(&encoded).unwrap();
    assert_eq!(decoded, script_code);

    // And we can mix these to calls because both include the length prefix.
    let encoded = encode::serialize_hex(&script_code);
    let decoded = ScriptBuf::<ScriptPubkey>::from_hex_prefixed(&encoded).unwrap();
    assert_eq!(decoded, script_code);
    ...
    let decoded: ScriptBuf<ScriptPubkey> = encode::deserialize(&encoded).unwrap();
    assert_eq!(decoded, script_code);
