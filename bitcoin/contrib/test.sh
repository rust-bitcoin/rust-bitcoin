...
then
    ...
    cargo --locked clippy --example bip32 -- -D warnings
    cargo --locked clippy --example handshake --features=rand-std -- -D warnings
    cargo --locked clippy --example ecdsa-psbt --features=bitcoinconsensus -- -D warnings
    cargo --locked clippy --example taproot-psbt --features=rand-std,bitcoinconsensus -- -D warnings
fi

echo "********* Testing std *************"
# Test without any features other than std first
cargo --locked test --verbose --no-default-features --features="std"
...
then
    ...
    cargo --locked build --verbose --features="no-std" --no-default-features

    # Build std + no_std, to make sure they are not incompatible
    cargo --locked build --verbose --features="no-std"

    # Test no_std
    cargo --locked test --verbose --features="no-std" --no-default-features

    # Build all features
    cargo --locked build --verbose --features="no-std $FEATURES" --no-default-features

    # Build specific features
    for feature in ${FEATURES}
    do
        cargo --locked build --verbose --features="no-std $feature" --no-default-features
    done

    cargo --locked run --example bip32 7934c09359b234e076b9fa5a1abfd38e3dc2a9939745b7cc3c22a48d831d14bd
    cargo --locked run --no-default-features --features no-std --example bip32 7934c09359b234e076b9fa5a1abfd38e3dc2a9939745b7cc3c22a48d831d14bd
fi
...
do
    ...
    cargo --locked test --verbose --features="$feature"
done

cargo --locked run --example ecdsa-psbt --features=bitcoinconsensus
cargo --locked run --example taproot-psbt --features=rand-std,bitcoinconsensus
