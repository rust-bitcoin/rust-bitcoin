...
use secp256k1::{schnorr, Keypair, Message};
        ...
        for (&xonly, (leaf_hashes, key_source)) in input.tap_key_origins.iter() {
            ...
            if let Some(internal_key) = input.tap_internal_key {
                ...
                if internal_key == xonly && leaf_hashes.is_empty() && input.tap_key_sig.is_none() {
                    ...
                    let signature =
                        schnorr::sign_no_aux_rand(&sighash.to_byte_array(), &key_pair);
            ...
            if let Some((leaf_hashes, _)) = input.tap_key_origins.get(&xonly) {
                ...
                if !leaf_hashes.is_empty() {
                    ...
                    for lh in leaf_hashes {
                        ...
                        #[cfg(feature = "rand-std")]
                        let signature = schnorr::sign(&sighash.to_byte_array(), &key_pair);
                        #[cfg(not(feature = "rand-std"))]
                        let signature =
                            schnorr::sign_no_aux_rand(&sighash.to_byte_array(), &key_pair);
...
mod tests {
    ...
    fn get_key_xpriv_bip32_parent() {
        ...
        let derived_key = derived_xpriv
            .get_key(&KeyRequest::Bip32((parent_xpriv.fingerprint(), path.clone())))
            .unwrap();
