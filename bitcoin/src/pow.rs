...
{
    let adjustment_interval = u32::try_from(params.difficulty_adjustment_interval() & u64::from(u32::MAX)).unwrap();

    if !is_retarget_height(current_height.increment(), adjustment_interval) {
        if params.allow_min_difficulty_blocks { // Only true for testnet and regtest.
            ...
            if new_block_timestamp > current_header.time.to_u32() + pow_target_spacing * 2 {
                ...
                while header.prev_blockhash != BlockHash::from_byte_array([0u8; 32])
