...
impl<'a> Arbitrary<'a> for InputWeightPrediction {
    fn arbitrary(u: &mut Unstructured<'a>) -> arbitrary::Result<Self> {
        ...
        let count: usize = witness_element_lengths.clone().into_iter().sum();
        ...
        witness_element_lengths = witness_element_lengths
            .iter()
            .take_while(|elem| {
                let size = total_size.checked_add(**elem);

                if let Some(s) = size {
                    if let Some(t) = s.checked_add(compact_size::encoded_size(count)) {
                        total_size = t;
                        return true;
                    }
                }
