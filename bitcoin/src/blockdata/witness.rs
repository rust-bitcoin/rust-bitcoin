...
// TODO: These should be re-exported in this crate.
use primitives::witness::WitnessDecoder;
...
impl Decodable for Witness {
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, Error> {
        use crate::consensus::ParseError;
        
        let mut decoder = WitnessDecoder::default();

        loop {
            let mut buffer = match r.fill_buf() {
                Ok(buffer) => buffer,
            ...
            if buffer.is_empty() {
                ...
                return decoder.end().map_err(|_| Error::Parse(ParseError::ParseFailed("witness")));
            }

            let original_len = buffer.len();
            let need_more = decoder.push_bytes(&mut buffer).map_err(|_| Error::Parse(ParseError::ParseFailed("witness")))?;
