...
fn bitcoinconsensus() {
    ...
    spent.verify(0, Amount::from_sat(18393430), &spending).unwrap();
}
...
fn default_dust_value() {
    ...
    assert_eq!(script_p2wpkh.minimal_non_dust(), Amount::from_sat(294));
    assert_eq!(
    ...
    assert_eq!(script_p2pkh.minimal_non_dust(), Amount::from_sat(546));
    assert_eq!(
        script_p2pkh.minimal_non_dust_custom(FeeRate::from_sat_per_vb_unchecked(6)),
        Amount::from_sat(1092)
    );
}
