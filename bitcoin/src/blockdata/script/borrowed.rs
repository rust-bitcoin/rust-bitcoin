...
use super::{
    Builder, Context, Instruction, InstructionIndices, Instructions, PushBytes, RedeemScript,
    RedeemScriptSizeError, ScriptHash, ScriptPubkey, ScriptSig, TapScript, WScriptHash,
...
pub trait ScriptExt: sealed::Sealed {
    ...
    /// changes.
    fn is_standard_op_return(&self) -> bool;
    ...
    fn to_hex_string_prefixed(&self) -> String;
...
impl<C: Context> ScriptExt for Script<C> {
    #[inline]
    ...
    fn to_hex_string_no_length_prefix(&self) -> String { self.as_bytes().to_lower_hex_string() }

    fn first_opcode(&self) -> Option<Opcode> { self.as_bytes().first().copied().map(From::from) }
...
crate::internal_macros::define_extension_trait! {
    /// Extension functionality for the [`Script`] type.
    pub trait ScriptExtScriptPubkey impl for Script<ScriptPubkey> {
...
crate::internal_macros::define_extension_trait! {
    /// Extension functionality for the [`Script`] type.
    pub trait ScriptExtScriptSig impl for Script<ScriptSig> {
        /// Constructs a new script builder
        fn builder() -> Builder<ScriptSig> { Builder::new() }
        ...
        fn redeem_script(&self) -> Option<&Script<RedeemScript>> {
            ...
            if let Some(Ok(Instruction::PushBytes(b))) = self.instructions().last() {
                Some(Script::<RedeemScript>::from_bytes(b.as_bytes()))
            } else {
                None
            }
        }
...
crate::internal_macros::define_extension_trait! {
    /// Extension functionality for the [`Script`] type.
    pub trait ScriptExtRedeemScript impl for Script<RedeemScript> {
        /// Constructs a new script builder
        fn builder() -> Builder<RedeemScript> { Builder::new() }
...
crate::internal_macros::define_extension_trait! {
    /// Extension functionality for the [`Script`] type.
    pub trait ScriptExtWitnessScript impl for Script<WitnessScript> {
        /// Constructs a new script builder
        fn builder() -> Builder<WitnessScript> { Builder::new() }
...
crate::internal_macros::define_extension_trait! {
    /// Extension functionality for the [`Script`] type.
    pub trait ScriptExtTapScript impl for Script<TapScript> {
        /// Constructs a new script builder
        fn builder() -> Builder<TapScript> { Builder::new() }
...
impl<C: Context> ScriptExtPriv for Script<C> {
    fn count_sigops_internal(&self, accurate: bool) -> usize {
        let mut n = 0;
...
crate::internal_macros::define_extension_trait! {
    pub(crate) trait ScriptExtPrivScriptPubkey impl for Script<ScriptPubkey> {
