    ...
    while let Some((pos, mut word)) = words.next() {
        ...
        if let Ok(op) = Opcode::from_str(word) {
            ...
            if op.code <= OP_PUSHDATA4.code {
                let (next, push) = words.next().ok_or(err(pos, ParseAsmErrorKind::UnexpectedEOF))?;
                ...
                let expected_push_op = match buf.len() as u64 {
                    n if n < opcodes::OP_PUSHDATA1.code as u64 => {
                        Opcode::from(n as u8)
                    }
                    n if n < 0x100 => {
