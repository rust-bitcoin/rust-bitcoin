    ...
    fn spentscript(&mut self, idx: usize, tx: &Transaction, prev_script: &Script) -> sha256::Hash {
        ...
        *self.spentscripts[idx].get_or_insert_with(|| {
            if prev_script.is_p2sh() {
                ...
                let ts = tx.input[idx].witness.tapscript().expect("p2tr");
...
mod test {
    ...
    fn test_txfs_templates() {
        assert_eq!("fff6bfbf", TXFS_SPECIAL_TEMPLATE.as_hex().to_string());
        assert_eq!("ffffbfbf", TXFS_SPECIAL_ALL.as_hex().to_string());
    }
        ...
        fn test_vectors() {
            for case in read_vector_file() {
                ...
                for v in case.vectors {
                    println!("txfs: {}; input: {}", v.txfs.as_hex(), v.input);
