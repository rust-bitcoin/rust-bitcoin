...
use hashes::{sha256, sha256d, sha256t};
...
impl Decodable for sha256d::Hash {
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, Error> {
        Ok(Self::from_byte_array(<[u8; sha256d::DIGEST_SIZE]>::consensus_decode(r)?))
    }
}
...
impl Decodable for sha256::Hash {
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, Error> {
        Ok(Self::from_byte_array(<[u8; sha256::DIGEST_SIZE]>::consensus_decode(r)?))
    }
}
...
impl Decodable for TapLeafHash {
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, Error> {
        Ok(Self::from_byte_array(<[u8; sha256t::DIGEST_SIZE]>::consensus_decode(r)?))
    }
}
