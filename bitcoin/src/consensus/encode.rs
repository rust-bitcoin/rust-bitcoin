...
pub use super::{Error, FromHexError, InvalidError, DeserError};
...
pub fn deserialize<T: Decodable>(data: &[u8]) -> Result<T, DeserError> {
...
/// doesn't consume the entire vector.
pub fn deserialize_partial<T: Decodable>(data: &[u8]) -> Result<(T, usize), InvalidError> {
    let mut decoder = Cursor::new(data);

    let rv = match Decodable::consensus_decode_from_finite_reader(&mut decoder) {
        Ok(rv) => rv,
        Err(Error::Invalid(e)) => return Err(e),
        Err(Error::Io(_)) =>
...
mod tests {
    ...
    fn deserialize_vec_test() {
        ...
        assert_eq!(err, DeserError::Invalid(InvalidError::MissingData));
