...
use super::{Decodable, Encodable, Error, InvalidError};
...
fn consensus_error_into_serde<E: serde::de::Error>(error: InvalidError) -> E {
    match error {
        InvalidError::MissingData =>
            E::custom("missing data (early end of file or slice too short)"),
        InvalidError::OversizedVectorAllocation { requested, max } => E::custom(format_args!(
            "the requested allocation of {} items exceeds maximum of {}",
            requested, max
        )),
        InvalidError::InvalidChecksum { expected, actual } => E::invalid_value(
        ...
        InvalidError::NonMinimalVarInt =>
            E::custom(format_args!("compact size was not encoded minimally")),
        InvalidError::ParseFailed(msg) => E::custom(msg),
        InvalidError::UnsupportedSegwitFlag(flag) =>
    ...
    E: serde::de::Error,
{
    fn unify(self) -> E {
        match self {
            ...
            DecodeError::Consensus(Error::Invalid(e)) => consensus_error_into_serde(e),
