...
pub mod hex {
    ...
    #[derive(Debug)]
    pub struct DecodeInitError(internals::hex::Error);

    /// Error returned when a hex string contains invalid characters.
    #[derive(Debug)]
    pub struct DecodeError(internals::hex::Error);

    /// Hex decoder state.
    pub struct Decoder<'a>(internals::hex::HexIterator<'a>);

    impl<'a> Decoder<'a> {
        fn new(s: &'a str) -> Result<Self, DecodeInitError> {
            match internals::hex::HexIterator::new(s) {
