...
pub use normal::*;
pub use normal_derivation_path::{
    IntoIter as NormalDerivationPathIntoIter, Iter as NormalDerivationPathIter,
...
/// A BIP32 types of key derivation
#[derive(Copy, Clone, PartialEq, Eq, Debug, PartialOrd, Ord, Hash)]
pub enum ChildNumber {
    /// Enum wrapper for a normal child number
    Normal(NormalChildNumber),
    /// Enum wrapper for a hardened child number
...
mod hardened {
    ...
    impl HardenedChildNumber {
        ...
        /// Consumes the HardenedChildNumber and convert it to ChildNumber::Hardened.
...
///Sub-module for NormalChildNumber type
mod normal {
    use super::*;
...
impl fmt::Display for HardenedChildNumber {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result { write!(f, "{}'", self.to_raw_number()) }
}
...
impl ChildNumber {
    ...
    /// Create a [`HardenedChildNumber`] from an index, returns an error if the index is not within
    /// [0, 2^31 - 1].
    ///
    ...
    pub fn from_hardened_idx(index: u32) -> Result<Self, Error> {
        ...
        //Ok(ChildNumber::Normal(HardenedChildNumber::new(index)?))
    }
    ...
    pub fn from_raw(index: u32) -> ChildNumber {
        if index & (1 << 31) != 0 {
            ChildNumber::Hardened(
                HardenedChildNumber::from_index(index ^ (1 << 31))
                    .expect("Unreachable; Index is Always Valid"),
            )
        } else {
            ChildNumber::Normal(
                NormalChildNumber::from_index(index).expect("Unreachable; Index is Always Valid"),
            )
        }
    }
}
...
impl From<u32> for ChildNumber {
    fn from(index: u32) -> Self { Self::from_raw(index) }
}
...
pub mod normal_derivation_path {
    ...
    ///A DerivationPath that can only contains NormalChildNumbers (i.e. no hardened indexes)
    ...
    impl NormalDerivationPath {
        ...
        #[allow(dead_code)]
        const MASTER: Self = NormalDerivationPath(Vec::new());

        /// Create a new `NormalDerivationPath` from a vector of `NormalChildNumber`
        ...
        /// Create a new [NormalDerivationPath] that is a child of this one.
        pub fn child(&self, cn: NormalChildNumber) -> Self { self.new_extended_from([cn]) }

        /// Convert into a [NormalDerivationPath] that is a child of this one.
        pub fn into_with_child(self, cn: NormalChildNumber) -> Self {
            let mut path = self;
            path.extend_from([cn]);
            path
        }
        /// Concatenate `path` to `self`.
        ///
        /// ```
        /// use bitcoin::bip32::{NormalDerivationPath, NormalChildNumber};
        /// use std::str::FromStr;
        ///
        /// let mut base = NormalDerivationPath::from_str("m/42").unwrap();
        ///
        /// base.extend_from(NormalDerivationPath::from_str("0/1").unwrap());
        /// base.extend_from([
        ///     NormalChildNumber::ZERO,
        ///     NormalChildNumber::ONE
        /// ]);
        ///
        /// assert_eq!(base, NormalDerivationPath::from_str("m/42/0/1/0/1").unwrap());
        /// ```

        pub fn extend_from<T: IntoIterator<Item = NormalChildNumber>>(&mut self, path: T) {
            self.extend(path);
        }
        ...
        pub fn new_extended_from<T: IntoIterator<Item = NormalChildNumber>>(
            &self,
            path: T,
        ) -> NormalDerivationPath {
            let mut ret = self.clone();
            ret.extend_from(path);
            ret
        }
    ...
    /// Normal Derivation Path Iterator Wrapper
    ...
    /// Normal Derivation Path IntoIterator Wrapper
...
/// Derivation path methods and utils.
pub mod derivation_path {
    ...
    impl DerivationPath {
        ...
        /// Create a new `DerivationPath` from a vector of `ChildNumber`.
        pub fn new(vec: Vec<ChildNumber>) -> Self { DerivationPath(vec) }
        /// Try to convert this DerivationPath into a NormalDerivationPath
        pub fn to_normal(&self) -> Result<NormalDerivationPath, Error> {
            NormalDerivationPath::try_from(self.0.as_slice())
        }
        /// Check if the derivation path is empty
        pub fn is_empty(&self) -> bool { self.0.is_empty() }
        ...
        ///
        /// base.extend_from(DerivationPath::from_str("0/1").unwrap());
        /// base.extend_from([
        ///     ChildNumber::ZERO_NORMAL,
        ///     ChildNumber::ONE_NORMAL
        /// ]);
        ///
        /// assert_eq!(base, DerivationPath::from_str("m/42/0/1/0/1").unwrap());
        /// ```

        pub fn extend_from<T: IntoIterator<Item = ChildNumber>>(&mut self, path: T) {
            self.extend(path);
        }
        /// Concatenate `self` with `path` and return the resulting new path.
        ///
        ...
        /// assert_eq!(deriv_1, deriv_2);
        /// ```
        pub fn new_extended_from<T: IntoIterator<Item = ChildNumber>>(
            &self,
            path: T,
        ) -> DerivationPath {
            let mut ret = self.clone();
    ...
    /// Derivation Path Iterator Wrapper
    ...
    /// Derivation Path Into Iterator Wrapper
...
impl Xpriv {
    ...
    pub fn derive_priv<C: secp256k1::Signing, P: core::iter::IntoIterator<Item = ChildNumber>>(
        &self,
        secp: &Secp256k1<C>,
        path: P,
    ) -> Xpriv {
...
impl Xpub {
    ...
    /// Derive an extended public key from a path that contains NormalChildNumbers.
    pub fn derive_pub<
    ...
    ///
    /// The `path` argument can be any type implementing `IntoIterator<Item = &'a ChildNumber>`, such as `DerivationPath`, for instance.
...
mod tests {
    ...
    fn test_path<C: secp256k1::Signing + secp256k1::Verification>(
        ...
        //} else {
        //    assert_eq!(&pk.derive_pub(secp, path.into()).to_string()[..], expected_pk);
        //}
