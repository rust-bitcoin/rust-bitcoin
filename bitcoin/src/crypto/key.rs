...
impl FromStr for PublicKey {
    ...
    fn from_str(s: &str) -> Result<PublicKey, ParsePublicKeyError> {
        ...
        match s.len() {
            66 => {
                let bytes = hex_stable::decode_array::<33>(s).map_err(|e| match e.parse_error() {
            ...
            130 => {
                let bytes = hex_stable::decode_array::<65>(s).map_err(|e| match e.parse_error() {
                    InvalidChar(e) => ParsePublicKeyError::InvalidChar(e),
                    InvalidLength(_) => unreachable!("length checked already"),
                })?;
                Ok(PublicKey::from_slice(&bytes)?)
...
pub enum ParseCompressedPublicKeyError {
    ...
    Hex(hex_stable::HexToArrayError),
}
...
impl From<hex_stable::HexToArrayError> for ParseCompressedPublicKeyError {
    fn from(e: hex_stable::HexToArrayError) -> Self { Self::Hex(e) }
}
