...
impl Keypair {
    ...
    #[inline]
    pub fn to_secret_key(self) -> secp256k1::SecretKey { secp256k1::SecretKey::from_keypair(&self.0) }
    ...
    /// # Errors
    ///
    /// Returns a [`secp256k1::Error`] error if the resulting key would be invalid.
    ///
    /// NB: Will not error if the tweaked public key has an odd value and can't be used for
    ///     BIP 340-342 purposes.
    #[inline]
    fn add_xonly_tweak(self, tweak: &secp256k1::Scalar) -> Result<Self, secp256k1::Error> {
        self.0.add_xonly_tweak(tweak).map(Self::from)
    }
}
...
impl TweakedPublicKey {
    ...
    pub fn from_keypair(keypair: TweakedKeypair) -> Self {
        let (xonly, _parity) = keypair.0.to_x_only_public_key();
        Self(xonly)
    }
...
impl TweakedKeypair {
    ...
    pub fn public_parts(&self) -> (TweakedPublicKey, Parity) {
        let (xonly, parity) = self.0.to_x_only_public_key();
