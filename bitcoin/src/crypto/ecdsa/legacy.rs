...
/// A legacy Bitcoin ECDSA signature.
///
/// This is just a thin wrapper around [`ecdsa::Signature`].
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "serde", serde(crate = "actual_serde"))]
pub struct LegacySignature(pub(crate) ecdsa::Signature);

impl LegacySignature {
    /// Construct a legacy ECDSA Bitcoin signature for [`EcdsaSighashType::All`].
    pub fn sighash_all(sig: secp256k1::ecdsa::Signature) -> LegacySignature {
    ...
    /// Serializes a legacy ECDSA signature (inner secp256k1 signature in DER format).
    ///
    /// This does **not** perform extra heap allocation.
    pub fn serialize(&self) -> SerializedSignature { self.0.serialize() }

    /// Serializes a legacy ECDSA signature (inner secp256k1 signature in DER format) into `Vec`.
    ///
...
impl fmt::Display for LegacySignature {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { fmt::Display::fmt(&self.0, f) }
}

impl FromStr for LegacySignature {
    type Err = Error;
