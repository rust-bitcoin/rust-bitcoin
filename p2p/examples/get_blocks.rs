...
fn main() {
    ...
    // requests headers, and then requests the actual data for the first few blocks.
    let args: Vec<String> = env::args().collect();
    if args.len() < 2 {
        eprintln!("usage: cargo run --example get_blocks -- <address> [network]");
        process::exit(1);
    }
        ...
        loop {
            ...
            match reply.payload() {
                ...
                message::NetworkMessage::Headers(headers_msg) => {
                    ...
                    if count > 0 {
                        // Request the first 25 blocks, but don't ask for more than we have headers for
                        let blocks_to_request = std::cmp::min(25, count);
                        println!("Requesting data for the first {} blocks...", blocks_to_request);

                        let mut inv_items = Vec::new();
                        for i in 0..blocks_to_request {
                            let hash = headers_msg.0[i].block_hash();
                            inv_items.push(message_blockdata::Inventory::Block(hash));
                        }

                        let get_data = InventoryPayload(inv_items);
                        let msg = message::RawNetworkMessage::new(
                            magic,
                        ...
                        println!("No headers received, cannot request blocks.");
                        break;
                    }
                }
                message::NetworkMessage::Block(block) => {
                    let hash = block.block_hash();
                    let (header, txs) = block.as_parts();
                    let tx_count = txs.len();
                    // Just print basic info, we don't want to dump the whole block to console
                    println!(
                        "Received BLOCK: {} | Transactions: {} | Bits: {:?}",
                        hash, tx_count, header.bits
                    );
                }
                _ => {
                    // Ignore other messages (ping, alert, addr, etc)
