...
impl Encodable for AddrV2 {
    fn consensus_encode<W: Write + ?Sized>(&self, w: &mut W) -> Result<usize, io::Error> {
        fn encode_addr<W: Write + ?Sized>(
            ...
            Ok(network.consensus_encode(w)? + crate::deser::consensus_encode_with_size(bytes, w)?)
        }
        Ok(match *self {
...
impl Decodable for AddrV2 {
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, encode::Error> {
        ...
        if len > 512 {
            return Err(crate::deser::parse_failed_error("IP must be <= 512 bytes"));
        }
        Ok(match network_id {
            1 => {
                if len != 4 {
                    return Err(crate::deser::parse_failed_error("invalid IPv4 address"));
                }
            ...
            2 => {
                if len != 16 {
                    return Err(crate::deser::parse_failed_error("invalid IPv6 address"));
                }
                let addr: [u16; 8] = read_be_address(r)?;
                if addr[0..3] == ONION {
                    return Err(crate::deser::parse_failed_error(
            ...
            4 => {
                if len != 32 {
                    return Err(crate::deser::parse_failed_error("invalid TorV3 address"));
                }
            ...
            5 => {
                if len != 32 {
                    return Err(crate::deser::parse_failed_error("invalid I2P address"));
                }
            ...
            6 => {
                if len != 16 {
                    return Err(crate::deser::parse_failed_error("invalid CJDNS address"));
                }
                let addr: [u16; 8] = read_be_address(r)?;
