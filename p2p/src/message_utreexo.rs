...
impl Decodable for UtreexoProof {
    ...
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, encode::Error> {
        ...
        Ok(UtreexoProof { blockhash, proof_hashes, target_locations, leaf_datas })
    }
}
...
impl Decodable for UtreexoSummary {
    ...
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, encode::Error> {
        ...
        Ok(UtreexoSummary { blockhash, num_additions, target_locations })
    }
}
...
impl Decodable for UtreexoRoot {
    ...
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, encode::Error> {
        ...
        Ok(UtreexoRoot { num_leaves, target, blockhash, root_hashes, proof_hashes })
    }
}
...
impl Decodable for GetUtreexoRoot {
    #[inline]
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, encode::Error> {
        Ok(GetUtreexoRoot { blockhash: BlockHash::consensus_decode(r)? })
    }
}
