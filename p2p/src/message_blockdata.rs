...
impl InventoryDecoder {
    fn err_from_inner(e: <InventoryInnerDecoder as encoding::Decoder>::Error) -> InventoryDecoderError {
        match e {
            encoding::Decoder2Error::First(f) => InventoryDecoderError::InventoryType(f),
            encoding::Decoder2Error::Second(s) => InventoryDecoderError::InventoryHash(s),
        }
    }
}
...
impl encoding::Decoder for InventoryDecoder {
    ...
    fn end(self) -> Result<Self::Output, Self::Error> {
        let (ty, inv) = self.0.end().map_err(Self::err_from_inner)?;
...
pub enum InventoryDecoderError {
    /// Error decoding the inventory type.
    InventoryType(encoding::UnexpectedEofError),
    /// Error decoding the inventory hash.
    InventoryHash(encoding::UnexpectedEofError),
}
...
impl fmt::Display for InventoryDecoderError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            Self::InventoryType(ref e) => write_err!(f, "inventory decoder error"; e),
            Self::InventoryHash(ref e) => write_err!(f, "inventory decoder error"; e),
        }
    }
}
...
impl std::error::Error for InventoryDecoderError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match *self {
            Self::InventoryType(ref e) => Some(e),
            Self::InventoryHash(ref e) => Some(e),
        }
    }
}
