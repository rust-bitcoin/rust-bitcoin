...
use encoding::{self, ArrayDecoder, ArrayEncoder, Decoder2, Encoder2};
use hashes::sha256d;
...
encoding::encoder_newtype! {
    ...
    pub struct NetworkHeaderEncoder(Encoder2<HeaderEncoder, ArrayEncoder<1>>);
}

impl encoding::Encodable for NetworkHeader {
    type Encoder<'e> = NetworkHeaderEncoder;

    fn encoder(&self) -> Self::Encoder<'_> {
        NetworkHeaderEncoder(Encoder2::new(
            self.header.encoder(),
