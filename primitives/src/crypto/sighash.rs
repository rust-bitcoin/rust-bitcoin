...
impl SighashTypeParseError {
    /// Creates a new error from an `unrecognized` string.
    pub fn new(unrecognized: String) -> Self { Self { unrecognized } }
}
...
impl<R: Borrow<Transaction>> SighashCache<R> {
    ...
    /// [`ScriptBuf::p2wpkh_script_code`], for p2wsh just pass in the witness script. (Also see
...
pub enum P2wpkhError {
    ...
    /// ScriptBuf is not a witness program for a p2wpkh output.
    NotP2wpkhScript,
}
...
impl<E> EncodeSigningDataResult<E> {
    ...
    /// ```rust
    /// # #[cfg(feature = "crypto")] {
    /// # use bitcoin_primitives::hashes::{Hash as _, HashEngine as _};
    /// # use bitcoin_primitives::hex::FromHex;
    /// # use bitcoin_primitives::{consensus, ScriptBuf, Transaction, LegacySighash, SighashCache};
    /// #
...
mod tests {
    ...
    fn legacy_sighash() {
        ...
        let data = include_str!("../../../bitcoin/tests/data/legacy_sighash.json");
    ...
    fn test_sighash_errors() {
        ...
        assert_eq!(
            c.legacy_signature_hash(10, &ScriptBuf::new(), 0u32),
            Err(InputsIndexError(IndexOutOfBoundsError {
                index: 10,
                length: 1
            }))
