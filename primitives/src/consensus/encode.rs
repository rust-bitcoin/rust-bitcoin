...
pub trait GenericEncodeVec {}

impl<T: Encodable + GenericEncodeVec> Encodable for Vec<T> {
    #[inline]
    fn consensus_encode<W: Write + ?Sized>(&self, w: &mut W) -> Result<usize, io::Error> {
        let mut len = 0;
...
impl<T: Decodable + GenericEncodeVec> Decodable for Vec<T> {
    #[inline]
...
pub trait GenericEncodeVecU32 {}

impl<T: Encodable + GenericEncodeVecU32> Encodable for Vec<(u32, T)> {
    #[inline]
    fn consensus_encode<W: Write + ?Sized>(&self, w: &mut W) -> Result<usize, io::Error> {
        let mut len = 0;
        len += VarInt(self.len() as u64).consensus_encode(w)?;
...
impl<T: Decodable + GenericEncodeVecU32> Decodable for Vec<(u32, T)> {
    #[inline]
    fn consensus_decode_from_finite_reader<R: io::BufRead + ?Sized>(
        r: &mut R,
    ) -> core::result::Result<Self, Error> {
...
impl GenericEncodeVec for BlockHash {}
impl GenericEncodeVec for block::Header {}
impl GenericEncodeVec for TxMerkleNode {}
impl GenericEncodeVec for Transaction {}
impl GenericEncodeVec for TxOut {}
impl GenericEncodeVec for TxIn {}
impl GenericEncodeVec for Vec<u8> {}
impl GenericEncodeVec for u64 {}
impl GenericEncodeVec for TapLeafHash {}
impl GenericEncodeVec for VarInt {}
