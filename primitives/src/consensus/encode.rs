...
use crate::block::{self, BlockHash, TxMerkleNode};
...
// FIXME: Sort out this monstrosity.

// We cannot call `impl_vec` from outside this crate because of the orphin rule
// (Encodable is here and Vec<T> is in stdlib.)

/// Dummy trait that triggers the blanket implementation of `Encodable`/`Decodable` for `Vec<T>`.
pub trait Trigger {}

impl<T: Encodable + Trigger> Encodable for Vec<T> {
    #[inline]
    fn consensus_encode<W: Write + ?Sized>(&self, w: &mut W) -> Result<usize, io::Error> {
        let mut len = 0;
...
impl<T: Decodable + Trigger> Decodable for Vec<T> {
    #[inline]
...
/// Dummy trait that triggers the blanket implementation of Encodable/Decodable for Vec<(u32, T)>.
pub trait TriggerU32 {}

impl<T: Encodable + Trigger> Encodable for Vec<(u32, T)> {
    #[inline]
    fn consensus_encode<W: Write + ?Sized>(&self, w: &mut W) -> Result<usize, io::Error> {
        let mut len = 0;
        len += VarInt(self.len() as u64).consensus_encode(w)?;
...
impl<T: Decodable + Trigger> Decodable for Vec<(u32, T)> {
    #[inline]
    fn consensus_decode_from_finite_reader<R: io::BufRead + ?Sized>(
        r: &mut R,
    ) -> core::result::Result<Self, Error> {
    ...
    ($type: ty) => {
        impl $crate::consensus::Encodable for Vec<$type> {
            ...
            fn consensus_encode<W: io::Write + ?Sized>(
                ...
                let mut len = 0;
                len += $crate::VarInt(self.len() as u64).consensus_encode(w)?;
                for c in self.iter() {
        ...
        impl $crate::consensus::Decodable for Vec<$type> {
            ...
            fn consensus_decode_from_finite_reader<R: io::BufRead + ?Sized>(
                ...
                let len = $crate::VarInt::consensus_decode_from_finite_reader(r)?.0;
                // Do not allocate upfront more items than if the sequence of type
                // occupied roughly quarter a block. This should never be the case
                // for normal data, but even if that's not true - `push` will just
                // reallocate.
                ...
                for _ in 0..len {
                    ret.push($crate::consensus::Decodable::consensus_decode_from_finite_reader(r)?);
                }
                Ok(ret)
            }
        }
...
pub fn consensus_encode_with_size<W: Write + ?Sized>(
    data: &[u8],
    w: &mut W,
...
/// FIXME: This used to be private.
pub struct ReadBytesFromFiniteReaderOpts {
    /// FIXME: This used to be private.
    pub len: usize,
    /// FIXME: This used to be private.
...
#[inline]
// FIXME: This used to be private.
pub fn read_bytes_from_finite_reader<D: Read + ?Sized>(
    d: &mut D,
...
impl<T: Encodable> Encodable for alloc::sync::Arc<T> {
    fn consensus_encode<W: Write + ?Sized>(&self, w: &mut W) -> Result<usize, io::Error> {
        (**self).consensus_encode(w)
    }
}
...
mod tests {
    ...
    fn deserialize_tx_hex() {
        let hex = include_str!("../../../bitcoin/tests/data/previous_tx_0_hex"); // An arbitrary transaction.
    ...
    fn deserialize_tx_hex_too_many_bytes() {
        ...
        let mut hex = include_str!("../../../bitcoin/tests/data/previous_tx_0_hex").to_string(); // An arbitrary transaction.
        hex.push_str("abcdef");
        assert!(matches!(
