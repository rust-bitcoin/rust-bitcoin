...
pub(crate) mod hex_codec {
    ...
    fn hex_write_with_case<T: Encodable + Decodable>(
        ...
        let total_len = len + if f.alternate() { 2 } else { 0 };

        // We pad for width, and truncate for precision, but not vice-versa
        let pad_width = f.width().unwrap_or(total_len);
        let trunc_width = f.precision().map_or(len, |v| v.saturating_sub(if f.alternate() { 2 } else { 0 }));
    ...
    impl<T: Encodable + Decodable> HexPrimitive<'_, T> {
        ...
        pub(crate) fn from_str(s: &str) -> Result<T, ParsePrimitiveError<T>> {
            ...
            for result in iter {
                ...
                buffer[index] = result.map_err(ParsePrimitiveError::from)?;
                index += 1;
            }
