...
#[doc(inline)]
pub use units::locktime::{relative::{Height, Time, TimeOverflowError}, mtp_height::MtpAndHeight};
...
///
/// let current_height = relative::Height::from(100);
/// let utxo_height = relative::Height::from(80);
///
/// let chain_state = relative::MtpAndHeight::new(current_height, timestamps);
/// let utxo_state = relative::MtpAndHeight::new(utxo_height, utxo_timestamps);
/// let locktime = relative::LockTime::Time(relative::Time::from_512_second_intervals(10));
///
/// // Check if locktime is satisfied
/// assert!(locktime.is_satisfied_by(chain_state, utxo_state));
/// ```
...
impl LockTime {
    ...
    /// # use bitcoin_primitives::relative::MtpAndHeight;
    ...
    /// let utxo_timestamps: [BlockTime; 11] = generate_timestamps(1_599_000_000, 200);
    ///
    /// let current_height = Height::from(100);
    /// let utxo_height = Height::from(80);
    ///
    ...
    pub fn is_satisfied_by(self, chain_state: MtpAndHeight, utxo_state: MtpAndHeight) -> bool {
        match self {
            LockTime::Blocks(blocks) => blocks.is_satisfied_by(chain_state, utxo_state),
            LockTime::Time(time) => time.is_satisfied_by(chain_state, utxo_state),
        }
    }
...
mod tests {
    ...
    fn test_locktime_chain_state() {
        ...
        let chain_state = MtpAndHeight::new(Height::from_height(100), timestamps);
        let utxo_state = MtpAndHeight::new(Height::from_height(80), utxo_timestamps);

        let lock1 = LockTime::Blocks(Height::from_height(10));
        assert!(lock1.is_satisfied_by(chain_state, utxo_state));

        let lock2 = LockTime::Blocks(Height::from_height(21));
        assert!(!lock2.is_satisfied_by(chain_state, utxo_state));

        let lock3 = LockTime::Time(Time::from_512_second_intervals(10));
        assert!(lock3.is_satisfied_by(chain_state, utxo_state));

        let lock4 = LockTime::Time(Time::from_512_second_intervals(20000));
        assert!(!lock4.is_satisfied_by(chain_state, utxo_state));

        assert!(LockTime::ZERO.is_satisfied_by(chain_state, utxo_state));
        assert!(LockTime::from_512_second_intervals(0).is_satisfied_by(chain_state, utxo_state));

        let lock6 = LockTime::from_seconds_floor(5000).unwrap();
        assert!(lock6.is_satisfied_by(chain_state, utxo_state));

        let max_height_lock = LockTime::Blocks(Height::MAX);
        assert!(!max_height_lock.is_satisfied_by(chain_state, utxo_state));

        let max_time_lock = LockTime::Time(Time::MAX);
        assert!(!max_time_lock.is_satisfied_by(chain_state, utxo_state));

        let max_chain_state = MtpAndHeight::new(Height::MAX, generate_timestamps(u32::MAX, 100));
        let max_utxo_state = MtpAndHeight::new(Height::MAX, generate_timestamps(u32::MAX, 100));
        assert!(!max_height_lock.is_satisfied_by(max_chain_state, max_utxo_state));
        assert!(!max_time_lock.is_satisfied_by(max_chain_state, max_utxo_state));
    }
}
