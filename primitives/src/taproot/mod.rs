...
#[cfg(feature = "crypto")]
pub use crate::{
    crypto::taproot::{SigFromSliceError, Signature},
    key::{TapTweak, TweakedPublicKey, UntweakedPublicKey, XOnlyPublicKey},
};
#[doc(inline)]
...
impl ControlBlock {
    ...
    /// # Errors
    ///
    /// - [`DecodeError::InvalidControlBlockSize`] if `sl` is not of size 1 + 32 + 32N for any N >= 0.
    /// - [`DecodeError::InvalidTaprootLeafVersion`] if first byte of `sl` is not a valid leaf version.
    /// - [`DecodeError::InvalidInternalKey`] if internal key is invalid (first 32 bytes after the parity byte).
    /// - [`DecodeError::InvalidMerkleTreeDepth`] if merkle tree is too deep (more than 128 levels).
    pub fn decode(sl: &[u8]) -> Result<ControlBlock, DecodeError> {
        if sl.len() < TAPROOT_CONTROL_BASE_SIZE
            || (sl.len() - TAPROOT_CONTROL_BASE_SIZE) % TAPROOT_CONTROL_NODE_SIZE != 0
        {
            return Err(DecodeError::InvalidControlBlockSize(sl.len()));
        }
        ...
        let internal_key = UntweakedPublicKey::from_slice(&sl[1..TAPROOT_CONTROL_BASE_SIZE])
            .map_err(DecodeError::InvalidInternalKey)?;
...
impl FutureLeafVersion {
    pub(self) fn from_consensus(version: u8) -> Result<FutureLeafVersion, InvalidLeafVersionError> {
        match version {
            TAPROOT_LEAF_TAPSCRIPT => unreachable!(
                "FutureLeafVersion::from_consensus should be never called for 0xC0 value"
            ),
            TAPROOT_ANNEX_PREFIX => Err(InvalidLeafVersionError(TAPROOT_ANNEX_PREFIX)),
            odd if odd & 0xFE != odd => Err(InvalidLeafVersionError(odd)),
...
impl LeafVersion {
    ...
    pub fn from_consensus(version: u8) -> Result<Self, InvalidLeafVersionError> {
        match version {
...
// TODO: Check all variants are used.
...
pub enum DecodeError {
    ...
    InvalidTaprootLeafVersion(InvalidLeafVersionError),
...
impl fmt::Display for DecodeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match *self {
            ...
            InvalidMerkleTreeDepth(ref e) => write_err!(f, "invalid merkle tree depth"; e),
            InvalidTaprootLeafVersion(ref e) => write_err!(f, "invalid leaf version"; e),
...
#[cfg(feature = "crypto-std")] // FIXME: Is this right?
impl std::error::Error for DecodeError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        use DecodeError::*;

        match *self {
            InvalidInternalKey(ref e) => Some(e),
...
#[cfg(feature = "crypto")]
impl From<InvalidLeafVersionError> for DecodeError {
    fn from(e: InvalidLeafVersionError) -> Self { Self::InvalidTaprootLeafVersion(e) }
}
...
/// Merkle tree depth must not be more than 128.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct InvalidLeafVersionError(u8);

internals::impl_from_infallible!(InvalidLeafVersionError);

impl fmt::Display for InvalidLeafVersionError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
#[cfg(feature = "std")]
impl std::error::Error for InvalidLeafVersionError {}
