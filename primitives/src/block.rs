...
impl Encodable for Block<Checked> {
    #[inline]
    fn consensus_encode<W: io::Write + ?Sized>(&self, w: &mut W) -> Result<usize, io::Error> {
        use crate::consensus_encoding::WriteExt as _;
...
impl Decodable for Block<Unchecked> {
    ...
    fn consensus_decode_from_finite_reader<R: io::Read + ?Sized>(
        r: &mut R,
    ) -> Result<Block, consensus_encoding::Error> {
    ...
    #[inline]
    fn consensus_decode<R: io::Read + ?Sized>(r: &mut R) -> Result<Block, consensus_encoding::Error> {
        let mut r = r.take(internals::ToU64::to_u64(consensus_encoding::MAX_VEC_SIZE));
