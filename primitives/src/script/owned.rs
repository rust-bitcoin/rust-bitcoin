...
/// script. It has a close relationship with its borrowed counterpart, [`ScriptBuf`].
///
/// Just as other similar types, this implements [`Deref`], so [deref coercions] apply. Also note
/// that all the safety/validity restrictions that apply to [`ScriptBuf`] apply to `ScriptBuf` as well.
///
...
impl ScriptBuf {
    ...
    // FIXME: This was private before.
    pub fn new_witness_program_unchecked<T: AsRef<PushBytes>>(
