...
///
/// *[See also the `bitcoinprimitives::script` module](crate::script).*
///
...
impl Script {
    ...
    #[inline]
    pub(in crate::script) fn p2pk_pubkey_bytes(&self) -> Option<&[u8]> {
        match self.len() {
        ...
        let sats = dust_relay_fee
            .checked_mul(if self.is_op_return() {
                ...
                32 + 4 + 1 + (107 / 4) + 4 + // The spend cost copied from Core
                    ...
                    self.consensus_encode(&mut io::sink()).expect("sinks don't error") as u64 // The serialized size of this script_pubkey
            } else {
