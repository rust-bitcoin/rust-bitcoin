...
use encoding::{
    self, CompactSizeDecoder, CompactSizeDecoderError, Decoder, LengthPrefixExceedsMaxError,
};
#[cfg(feature = "hex")]
...
impl WitnessDecoder {
    ...
    pub fn new() -> Self {
        Self {
            ...
            bytes_to_read: 0,
            empty: false, // TODO: Improve this kludge.
        }
    }

    /// Constructs a new 'empty' witness decoder.
    ///
    /// When decoding a [`TxIn`] the witness is empty but so that we can use the [`Decoder4`] we
    /// provide a 'dummy' decoder that returns `Ok(false)` immediately for `push_bytes` and
    /// `Witness::default()` for `end`.
    pub fn empty() -> Self {
        Self {
            prefix_decoder: None,
            buffer: Vec::new(),
