...
use consensus_encoding::{Decodable, Encodable, ReadExt as _, WriteExt as _};
#[cfg(feature = "hex")]
...
impl Decodable for Witness {
    fn consensus_decode<R: Read + ?Sized>(r: &mut R) -> Result<Self, consensus_encoding::Error> {
        ...
        if witness_elements > consensus_encoding::MAX_VEC_SIZE {
            return Err(consensus_encoding::ParseError::OversizedVectorAllocation {
                requested: witness_elements,
                max: consensus_encoding::MAX_VEC_SIZE,
            }
            .into());
        }
        if witness_elements == 0 {
            ...
            for i in 0..witness_elements {
                ...
                let required_len = cursor
                    ...
                    .ok_or(consensus_encoding::Error::Parse(consensus_encoding::ParseError::OversizedVectorAllocation {
                        requested: usize::MAX,
                        max: consensus_encoding::MAX_VEC_SIZE,
                    }))?
                ...
                if required_len > consensus_encoding::MAX_VEC_SIZE + witness_index_space {
                    return Err(consensus_encoding::ParseError::OversizedVectorAllocation {
