...
This release is massive, the biggest viable changes are to the `Hash` trait, including inherent
functions so you should not need to import it at all anymore for normal usage. Check out how we are
using `hashes` in `rust-bitcoin` if you want to understand the motivation for change. Enjoy!
