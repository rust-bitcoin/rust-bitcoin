    ...
    rust_version! {
        if >= 1.72 {
            ...
            unsafe fn process_block_simd_aarch64_intrinsics(&mut self) {
                ...
                // Load the current hash state into two 128‐bit vectors.
                ...
                // Process a single block
                {
                    ...
                    // Load message
                    msg0 = vld1q_u32(self.buffer.as_ptr().add(block_offset) as *const u32);
                    msg1 = vld1q_u32(self.buffer.as_ptr().add(block_offset + 16) as *const u32);
                    msg2 = vld1q_u32(self.buffer.as_ptr().add(block_offset + 32) as *const u32);
                    msg3 = vld1q_u32(self.buffer.as_ptr().add(block_offset + 48) as *const u32);
