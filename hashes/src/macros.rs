...
macro_rules! impl_bytelike_traits {
    ($ty:ident, $len:expr, $reverse:expr $(, $gen:ident: $gent:ident)*) => {
        $crate::impl_bytelike_stringly_traits!($ty, $len, $reverse $(, $gen: $gent)*);
...
/// Adds stringly trait impls for an bytelike type, requires "hex" feature.
///
/// Implements:
///
/// * `str::FromStr`
...
/// * `$gen: $gent` - generic type(s) and trait bound(s).
#[macro_export]
macro_rules! impl_bytelike_stringly_traits {
    ($ty:ident, $len:expr, $reverse:expr $(, $gen:ident: $gent:ident)*) => {
