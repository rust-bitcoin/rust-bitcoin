...
use bitcoin_consensus_encoding::{encode_to_vec, Decoder, Decodable};
use honggfuzz::fuzz;

/// Decodes a type T from a byte slice, returning an Option instead of a Result,
/// and returning None if the slice is not completely consumed by the decode
/// (the same as old_bitcoin::consensus::encode::deserialize).
fn decode_from_slice<T>(bytes: &[u8]) -> Option<T>
where
    T: Decodable,
{
    let mut decoder = <T as Decodable>::decoder();
    let mut remaining = bytes;
    let mut push_result: Option<bool> = Some(true);
    while !remaining.is_empty() {
        push_result = decoder.push_bytes(&mut remaining).ok();
        if let Some(needs_more) = push_result {
            // If we don't need more, break
            if !needs_more {
                // But if bytes remain, set our result to None to indicate failure.
            ...
            // If push_bytes errors, break
            break;
        }
    };
    push_result.and_then(|_| decoder.end().ok())
}
...
macro_rules! compare_encoding {
    ...
    ($data:expr, $new_ty:ty, $old_ty:ty) => {{
        ...
        let old_result: Option<$old_ty> = old_bitcoin::consensus::encode::deserialize($data).ok();
        let new_result: Option<$new_ty> = decode_from_slice($data);
