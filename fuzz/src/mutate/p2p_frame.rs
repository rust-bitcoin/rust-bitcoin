...
/// As [`checksum`], but using `bitcoin 0.32`'s hashes.
///
/// The two crates depend on different `bitcoin_hashes` versions, and under `--cfg=hashes_fuzz`
/// both replace SHA256 with a stub. The 0.32 target has to be handed this one for the checksum
/// it computes to agree with the one its decoder checks.
pub fn checksum_0_32(payload: &[u8]) -> [u8; 4] {
...
///
/// `checksum` selects which crate version's SHA256 to checksum with: [`checksum`] for the
/// workspace `p2p`, [`checksum_0_32`] for `bitcoin 0.32`.
...
mod tests {
    ...
    fn run(input: &[u8], max_size: usize, seed: u32, builtin_mutate: Mutate) -> Vec<u8> {
        ...
        let len = mutate_payload(&mut data, input.len(), max_size, seed, builtin_mutate, checksum);
    ...
    fn tiny_max_size_falls_back_to_the_built_in_mutator() {
        for max_size in 0..=HEADER_LEN {
            ...
            let len = mutate_payload(&mut data, input.len(), max_size, 0, identity, checksum);
            assert!(len <= max_size);
        }
    }
}
