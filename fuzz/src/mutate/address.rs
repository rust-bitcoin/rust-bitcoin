...
/// The base58check codec of one version of the `bitcoin` crate.
///
/// bech32 needs no such indirection: both crate versions share a single `bech32` instance, and
/// its checksum involves no hashing. base58's does, and the two versions hash with different
/// `bitcoin_hashes` releases which `--cfg=hashes_fuzz` stubs out separately, so each target
/// has to encode with its own.
pub struct Base58Codec {
    /// Encodes a payload, appending its checksum.
    pub encode: fn(&[u8]) -> String,
    /// Decodes a checksummed string, returning the payload without its checksum.
    pub decode: fn(&str) -> Option<Vec<u8>>,
}

/// The codec of the workspace `bitcoin` crate.
pub const BITCOIN: Base58Codec = Base58Codec {
    encode: |payload| bitcoin::base58::Base58CkString::encode_unbounded(payload).as_str().into(),
    decode: |s| bitcoin::base58::decode_check(s).ok(),
};

/// The codec of the `bitcoin 0.32` crate.
pub const BITCOIN_0_32: Base58Codec = Base58Codec {
    encode: bitcoin_0_32::base58::encode_check,
    decode: |s| bitcoin_0_32::base58::decode_check(s).ok(),
};
...
///
/// `base58` selects which crate version's base58check to encode with: [`BITCOIN`] for the
/// workspace crate, [`BITCOIN_0_32`] for `bitcoin 0.32`.
pub fn mutate_payload(
    ...
    let mut record = core::str::from_utf8(&data[..size])
        .ok()
        .and_then(|s| decode(s, base58))
        .unwrap_or_else(|| {
...
fn decode(s: &str, base58: &Base58Codec) -> Option<Record> {
...
fn encode(record: &Record, base58: &Base58Codec) -> Option<String> {
...
mod tests {
    ...
    fn run(input: &[u8], max_size: usize, seed: u32, builtin_mutate: Mutate) -> Vec<u8> {
        ...
        let len = mutate_payload(&mut data, input.len(), max_size, seed, builtin_mutate, &BITCOIN);
    ...
    fn decode_then_encode_is_the_identity() {
        for address in samples() {
            let record = decode(&address, &BITCOIN).expect("{address} should decode");
            assert_eq!(encode(&record, &BITCOIN).as_deref(), Some(address.as_str()));
        }
    }
    ...
    fn a_near_miss_keeps_its_lineage() {
        ...
        let address = encode(&record, &BITCOIN).expect("near miss should encode");

        let back = decode(&address, &BITCOIN).expect("a near miss should still rebuild a record");
    ...
    fn every_form_is_reachable() {
        ...
        for seed in 0..512 {
            ...
            if let Some(record) = core::str::from_utf8(&out).ok().and_then(|s| decode(s, &BITCOIN))
            {
    ...
    fn uppercase_addresses_are_produced_and_parse() {
        ...
        let address = encode(&record, &BITCOIN).expect("should encode");
    ...
    fn base58_codec_round_trips() {
        ...
        let encoded = (BITCOIN.encode)(&payload);

        assert_eq!((BITCOIN.decode)(&encoded).as_deref(), Some(&payload[..]));
    }
}
