...
///
/// See also [`impl_array_newtype_serde`] for `serde::Serialize`/`Deserialize` impls, which are
/// generated by a separate macro so that callers can independently `cfg`-gate them (e.g. behind
/// a `serde` feature) without this macro needing to know the name of that feature.
///
/// # Caller Requirements
///
/// Because this macro is exported from `bitcoin-internals` but generates code that is compiled
/// in the *calling* crate, the identifier `hex` must resolve at the macro's invocation site.
/// Typically this means having `hex-conservative` as a direct dependency, but any `use` that
/// brings a crate into scope under that exact name (e.g. re-exported from another dependency)
/// also works.
#[macro_export]
...
/// Implements `serde::Serialize` and `Deserialize` (using lowercase hex) for byte-based newtypes.
///
/// Split out from [`impl_array_newtype_stringify`] so that callers can `cfg`-gate this
/// invocation themselves (e.g. behind a `serde` feature) rather than this macro hardcoding a
/// particular feature name.
///
/// ```ignore
/// internals::impl_array_newtype_stringify!(MyType, 32);
/// #[cfg(feature = "serde")]
/// internals::impl_array_newtype_serde!(MyType, 32);
/// ```
///
/// # Caller Requirements
///
/// Because this macro is exported from `bitcoin-internals` but generates code that is compiled
/// in the *calling* crate, the identifier `serde` must resolve at the macro's invocation site.
/// Typically this means having `serde` as a direct dependency, but any `use` that brings a
/// crate into scope under that exact name (e.g. re-exported from another dependency) also
/// works.
#[macro_export]
