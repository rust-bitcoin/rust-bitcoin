...
//!
//! A basic filter commits to selected output and spent-output scriptPubKeys in a block. Its
//! elements are hashed into a block-specific range, sorted, and Golomb-Rice coded. See
//! [`BasicFilter`] for construction and query operations.
...
#[cfg(feature = "alloc")]
use primitives::{Block, BlockChecked, OutPoint, ScriptPubKey};
...
/// A validated [BIP-0158] basic block filter.
///
/// Filters created by `BasicFilter::from_block` are canonical.
/// Filters decoded by [`BasicFilter::from_bytes`] are structurally validated.
///
/// Queries are probabilistic and may return false positives, but never false negatives
/// when the filter was correctly constructed and queried with the corresponding block hash.
/// The byte container `B` may own or borrow the serialized filter.
///
/// [BIP-0158]: <https://github.com/bitcoin/bips/blob/master/bip-0158.mediawiki>
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct BasicFilter<#[cfg(feature = "alloc")] B = Vec<u8>, #[cfg(not(feature = "alloc"))] B> {
    bytes: B,
    element_count: u32,
    payload_offset: u8,
}
...
impl BasicFilter<Vec<u8>> {
    ...
    fn from_elements(block_hash: BlockHash, elements: &BTreeSet<Vec<u8>>) -> Self {
...
impl<B: AsRef<[u8]>> BasicFilter<B> {
    /// Parses and validates serialized BIP-0158 basic-filter bytes.
    ///
    /// Unused bits in the final byte are ignored for compatibility with
    /// Bitcoin Core. Extra full bytes, noncanonical `CompactSize` values,
    /// truncated filters, and arithmetic overflow are rejected.
    ///
    /// # Errors
    ///
    /// Returns an error if `bytes` is not a structurally valid basic filter.
    pub fn from_bytes(bytes: B) -> Result<Self, DecodeError> {
        let (element_count, payload_offset) = validate(bytes.as_ref())?;
        Ok(Self { bytes, element_count, payload_offset })
    }

    /// Returns whether any query element matches this filter.
    ///
    /// `block_hash` must identify the block for which this filter was constructed.
    /// Query elements are typically serialized scriptPubKeys.
    ///
    /// An empty query returns `false`.
    pub fn match_any<I, T>(&self, block_hash: BlockHash, query: I) -> bool
    where
    ...
    ///
    /// `blockhash` must identify the block for which this filter was constructed.
    /// Query elements are typically serialized scriptPubKeys.
    ///
    ...
    /// Returns the serialized BIP-0158 filter.
...
#[cfg(feature = "alloc")]
impl TryFrom<Vec<u8>> for BasicFilter<Vec<u8>> {
    type Error = DecodeError;

    fn try_from(bytes: Vec<u8>) -> Result<Self, Self::Error> { Self::from_bytes(bytes) }
}

impl<B: AsRef<[u8]>> AsRef<[u8]> for BasicFilter<B> {
...
mod tests {
    ...
    fn empty_filter() {
        ...
        let filter = BasicFilter::from_bytes([0].as_slice()).unwrap();
    ...
    fn accepts_decoded_values_outside_construction_range() {
        // BIP-0158 constrains constructed values to [0, N*M), but, like Bitcoin Core,
        // decoding is structural and accepts values at or above that construction range.
