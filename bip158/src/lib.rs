...
use bitcoin::block::{Block, BlockHash, Checked};
use bitcoin::consensus::{ReadExt, WriteExt};
use bitcoin::script::{ScriptPubKey, ScriptPubKeyExt as _};
...
impl<'a, R: BufRead + ?Sized> BitStreamReader<'a, R> {
    ...
    ///
    /// ```
    /// # use bitcoin_bip158::BitStreamReader;
    /// # let data = [0xff];
