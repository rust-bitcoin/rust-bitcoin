...
//! Implements traits to support the following the calculations:
//!
//! - `fee_rate * weight = fee`
//! - `weight = fee / fee_rate`
//! - `fee_rate = fee / weight`
...
impl FeeRate {
    ...
    pub const fn checked_mul_by_weight(self, rhs: Weight) -> Option<Amount> {
        ...
        match self.to_sat_per_kwu().checked_mul(rhs.to_wu()) {
...
impl Amount {
    ...
    pub const fn checked_div_by_weight_floor(self, weight: Weight) -> Option<FeeRate> {
        // No `?` operator in const context.
        let fee = self.to_sat().checked_mul(1_000).expect("MAX_MONEY * 1000 cannot overflow a u64");
    ...
    ///
    /// Computes the weight for a transaction with `fee_rate` assuming this amount represents a
    /// transaction fee.
    ///
    ...
    #[cfg(feature = "alloc")]
    #[must_use]
    pub const fn checked_div_by_fee_rate_floor(self, _: FeeRate) -> Option<Weight> { todo!() }
}
...
impl ops::Div<Weight> for Amount {
    ...
    fn div(self, weight: Weight) -> Self::Output {
        self.checked_div_by_weight_floor(weight).unwrap()
    }
}
...
impl ops::Div<FeeRate> for Amount {
    ...
    fn div(self, fee_rate: FeeRate) -> Self::Output {
        self.checked_div_by_fee_rate_floor(fee_rate).unwrap()
    }
}
