...
impl SignedAmount {
    ...
    /// additional work will be larger than addition of a reasonably small multiple of 4392
    /// elements.
    ///
    ...
    fn sum_inner(amounts: &[Self]) -> Option<Self> {
        ...
        // i64::MAX / 21M btc gives us 4392, so summing up that many items is guaranteed not to
        ...
        // compiler can auto-vectorize this. This number also happens to be divisible by 8 (and
        // 24), so the compiler will nicely take advantage of it.
        let (chunks, remaining) = amounts.bitcoin_as_chunks::<4392>();
        // Notably add_unchecked is not faster - I already checked, it produces same assembly. Same
        // with assert_unchecked.
        for partial in chunks.iter().map(|chunk| chunk.iter().copied().map(Self::to_sat).sum::<i64>()) {
        ...
        // be divisible by 16.
        sum = sum.try_add_sat(remaining.iter().copied().map(Self::to_sat).sum::<i64>())?;
    ...
    #[inline(always)]
    fn try_add_sat(self, sats: i64) -> Option<Self> {
...
mod tests {
    ...
    #[test]
    fn sum() {
        let short = [SignedAmount::from_sat_i32(42); 1000];
        assert_eq!(SignedAmount::sum(&short).unwrap(), SignedAmount::from_sat_i32(42 * 1000));
        let exact = [SignedAmount::from_sat_i32(42); 8784];
        assert_eq!(SignedAmount::sum(&exact).unwrap(), SignedAmount::from_sat_i32(42 * 8784));
        let longer = [SignedAmount::from_sat_i32(42); 8784 + 30];
        assert_eq!(SignedAmount::sum(&longer).unwrap(), SignedAmount::from_sat_i32(42 * (8784 + 30)));
        let very_long = [SignedAmount::from_sat_i32(42); 8784 * 5 + 30];
        assert_eq!(SignedAmount::sum(&very_long).unwrap(), SignedAmount::from_sat_i32(42 * (8784 * 5 + 30)));
        let overflowing_short = [SignedAmount::MAX; 2];
        assert!(SignedAmount::sum(&overflowing_short).is_error());
        let overflowing_exact = [SignedAmount::MAX; 8784];
        assert!(SignedAmount::sum(&overflowing_exact).is_error());
        let overflowing_longer = [SignedAmount::MAX; 8784 + 30];
        assert!(SignedAmount::sum(&overflowing_longer).is_error());
        let overflowing_very_long = [SignedAmount::MAX; 8784 * 5 + 30];
        assert!(SignedAmount::sum(&overflowing_very_long).is_error());
    }
}
