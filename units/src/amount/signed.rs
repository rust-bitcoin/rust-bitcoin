...
impl SignedAmount {
    ...
    pub fn from_str_in(s: &str, denom: Denomination) -> Result<SignedAmount, ParseAmountError> {
        match parse_signed_to_satoshi(s, denom).map_err(|error| error.convert(true))? {
            // (negative, amount)
            (false, sat) if sat > SignedAmount::MAX.0 as u64 => Err(ParseAmountError(
                ParseAmountErrorInner::OutOfRange(OutOfRangeError::too_big(true)),
            )),
            (false, sat) => Ok(SignedAmount(sat as i64)),
            (true, sat) if sat > SignedAmount::MAX.0 as u64 => Err(ParseAmountError(
    ...
    /// Checks if the amount is above the minimum and below the maximum value.
    /// Returns `None` if it is outside the allowed range.
    const fn check_min_max(self) -> Option<SignedAmount> {
        if self.0 > Self::MAX.0 || self.0 < Self::MIN.0 {
            None
        } else {
            Some(self)
        }
