...
#[cfg(feature = "encoding")]
impl From<Infallible> for AmountDecodeError {
    fn from(never: Infallible) -> Self { match never {} }
}

#[cfg(feature = "encoding")]
impl From<encoding::UnexpectedEofError> for AmountDecodeError {
    fn from(e: encoding::UnexpectedEofError) -> Self { Self::UnexpectedEof(e) }
}

#[cfg(feature = "encoding")]
impl fmt::Display for AmountDecodeError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
...
impl std::error::Error for AmountDecodeError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match *self {
