...
///
/// For some types in this crate overflow can be catastropic (e.g. `Amount`). In order to prevent
/// overflow we provide a custom result type that is similar to the normal [`core::result::Result`]
/// but implements mathematical operatons (e.g. [`core::ops::Add`]) so that math operations can be
...
/// # Examples
///
/// The `NumOpResult` type provides overflow protection with two primary use cases.
///
/// ## Overflow protection
///
/// ```
...
///
/// In some instances one may wish to differentiate div-by-zero from other overflows.
///
/// ```
/// # use bitcoin_units::{amount, Amount, FeeRate, NumOpResult};
/// // Max fee we want to pay.
/// let max_fee = Amount::from_sat(1_000)?;
...
///     NumOpResult::Error(e) => {
///         if e.operation().is_div_by_zero() {
///             // Do something when div by zero.
///         };
///
///         if e.operation().is_overflow() {
///             // We separate div-by-zero from overflow in case it needs to be handled separately.
///             //
///             // This branch could be hit if `max_fee` came from some previous calculation. And if
///             // part of that calculation was from the user then overflow could be an attack vector.
///         }
///         unreachable!();
///     }
/// };
