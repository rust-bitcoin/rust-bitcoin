...
mod tests {
    ...
    fn mathop_expect() {
        ...
        for amount in amounts {
            assert_eq!(
                NumOpResult::Valid(amount).expect(""),
                NumOpResult::Valid(amount).unwrap(),
            );
            assert_eq!(NumOpResult::Valid(amount).expect(""), amount);
        }
    }
    ...
    #[test]
    #[should_panic(expected = "tried to unwrap an invalid numeric result")]
    ...
    #[test]
    #[should_panic(expected = "tried to unwrap a valid numeric result")]
