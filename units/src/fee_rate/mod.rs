...
mod tests {
    ...
    fn checked_add() {
        ...
        let f = FeeRate::from_sat_per_mvb(u64::MAX).checked_add(one);
        assert!(f.is_none());
    }
    ...
    fn fee_rate_from_sat_per_vb() {
        let f = FeeRate::from_sat_per_vb(10).expect("expected feerate in sat/kwu");
        assert_eq!(f, FeeRate::from_sat_per_kwu(2500).unwrap());
    }

    #[test]
    fn fee_rate_from_sat_per_kvb() {
        let f = FeeRate::from_sat_per_kvb(11).unwrap();
        assert_eq!(f, FeeRate::from_sat_per_mvb(11_000));
    }
    ...
    fn from_sat_per_vb_u32() {
        let f = FeeRate::from_sat_per_vb_u32(10);
        assert_eq!(f, FeeRate::from_sat_per_kwu(2500).unwrap());
    }
    ...
    fn checked_mul() {
        let f =
            FeeRate::from_sat_per_kwu(10).unwrap().checked_mul(10).expect("expected feerate in sat/kwu");
        assert_eq!(f, FeeRate::from_sat_per_kwu(100).unwrap());

        let f = FeeRate::from_sat_per_kwu(10).unwrap().checked_mul(u64::MAX);
        assert!(f.is_none());
    }
    ...
    fn checked_div() {
        let f =
            FeeRate::from_sat_per_kwu(10).unwrap().checked_div(10).expect("expected feerate in sat/kwu");
        assert_eq!(f, FeeRate::from_sat_per_kwu(1).unwrap());
