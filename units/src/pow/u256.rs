...
pub(super) struct U256(pub(super) u128, pub(super) u128);

impl U256 {
    pub(super) const MAX: Self =
    ...
    pub(super) const ONE: Self = Self(0, 1);

    /// Constructs a new `U256` from a prefixed hex string.
    pub(super) fn from_hex(s: &str) -> Result<Self, PrefixedHexError> {
    ...
    pub(super) fn from_unprefixed_hex(s: &str) -> Result<Self, UnprefixedHexError> {
        let checked = parse_int::hex_check_unprefixed(s)?;
        Ok(Self::from_hex_internal(checked)?)
    }

    // Caller to ensure `s` does not contain a prefix.
    pub(super) fn from_hex_internal(s: &str) -> Result<Self, ParseIntError> {
    ...
    pub(super) fn from_be_bytes(a: [u8; 32]) -> Self {
    ...
    pub(super) fn from_le_bytes(a: [u8; 32]) -> Self {
        let (high, low) = split_in_half(a);
    ...
    pub(super) fn is_zero(&self) -> bool { self.0 == 0 && self.1 == 0 }

    pub(super) fn is_one(&self) -> bool { self.0 == 0 && self.1 == 1 }

    pub(super) fn is_max(&self) -> bool { self.0 == u128::MAX && self.1 == u128::MAX }

    /// Returns the low 32 bits.
    pub(super) fn low_u32(&self) -> u32 { self.low_u128() as u32 }

    /// Returns the low 64 bits.
    pub(super) fn low_u64(&self) -> u64 { self.low_u128() as u64 }

    /// Returns the low 128 bits.
    pub(super) fn low_u128(&self) -> u128 { self.1 }
    ...
    pub(super) fn saturating_to_u128(&self) -> u128 {
    ...
    pub(super) fn mul_u64(self, rhs: u64) -> (Self, bool) {
    ...
    pub(super) fn div_rem(self, rhs: Self) -> (Self, Self) {
    ...
    /// overflow would occur. If an overflow would have occurred then the wrapped value is returned.
    #[must_use = "this returns the result of the operation, without modifying the original"]
    pub(super) fn overflowing_sub(self, rhs: Self) -> (Self, bool) {
    ...
    #[must_use = "this returns the result of the operation, without modifying the original"]
    pub(super) fn overflowing_mul(self, rhs: Self) -> (Self, bool) {
        let mut ret = Self::ZERO;
        let mut ret_overflow = false;
    ...
    pub(super) fn wrapping_inc(&self) -> Self {
        let mut ret = Self::ZERO;
    ...
    #[must_use = "this returns the result of the operation, without modifying the original"]
    pub(super) fn wrapping_shl(self, rhs: u32) -> Self {
    ...
    #[must_use = "this returns the result of the operation, without modifying the original"]
    pub(super) fn wrapping_shr(self, rhs: u32) -> Self {
        let shift = rhs & 0x0000_00ff;
    ...
    #[allow(clippy::indexing_slicing)]
    pub(super) fn fmt_decimal(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
mod tests {
    ...
    fn u256_lower_hex() {
        assert_eq!(
            alloc::format!("{:x}", U256::from(0xDEAD_BEEF_u64)),
    ...
    #[cfg(feature = "alloc")]
    fn u256_display() {
        assert_eq!(alloc::format!("{}", U256::from(100_u32)), "100",);
        assert_eq!(alloc::format!("{}", U256::ZERO), "0",);
        assert_eq!(alloc::format!("{}", U256::from(u64::MAX)), alloc::format!("{}", u64::MAX),);
        assert_eq!(
            alloc::format!("{}", U256::MAX),
    ...
    macro_rules! check_format {
        ($($test_name:ident, $val:literal, $format_string:literal, $expected:literal);* $(;)?) => {
            $(
                ...
                #[cfg(feature = "alloc")]
                fn $test_name() {
                    assert_eq!(alloc::format!($format_string, U256::from($val)), $expected);
                }
            )*
        }
    }
    ...
    fn u256_addition() {
        ...
        // overflow both in the low addition and the carry
        let (add, overflow) = U256::MAX.overflowing_add(U256::MAX);
        assert!(overflow);
        assert_eq!(add, U256(u128::MAX, u128::MAX - 1));

        // high overflows but low doesn't
        let max_high = U256(u128::MAX, 0);
        let (add, overflow) = max_high.overflowing_add(max_high);
        assert!(overflow);
        assert_eq!(add, U256(u128::MAX - 1, 0));
    }
    ...
    fn u256_to_f64() {
        ...
        // Kills mutants for - -> + in mantissa calculation with dropped_bits
        assert_eq!(U256(u128::MAX ^ 1 << 75, u128::MAX).to_f64(), 1.157_920_892_373_161_8e77_f64);

        // Edge case: value just below rounding threshold
        // Kills mutants with & with !mantissa
        let val5 = U256(0x0000_0000_0000_0001, 0x7FFF_FFFF_FFFF_FFFF);
        assert_eq!(val5.to_f64(), 3.402_823_669_209_385e38_f64);
    }

    #[test]
    fn u256_saturating_to_u128() {
        assert_eq!(U256::from(u128::MAX).saturating_to_u128(), u128::MAX);
        assert_eq!(U256(1, 0).saturating_to_u128(), u128::MAX);
        assert_eq!(U256(0, u128::MAX - 1).saturating_to_u128(), u128::MAX - 1);
    }
}
