...
/// A struct that includes all public non-error enums that implement `Hash` but not ordering.
/// C-COMMON-TRAITS: `Copy`, `Clone`, `Debug`, `PartialEq`, `Eq`, `Hash`
...
fn api_all_types_have_non_empty_debug() {
    ...
    assert!(!format!("{:?}", Amount::from_str("123").unwrap_err()).is_empty());
}
