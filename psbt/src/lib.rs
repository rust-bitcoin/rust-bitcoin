...
use bip32::{KeySource, Xpriv, Xpub};
use internals::write_err;
use primitives::key::TapTweak;
use primitives::secp256k1::{Keypair, Message, Secp256k1, Signing, Verification};
use primitives::sighash::{self, EcdsaSighashType, Prevouts, SighashCache, TapSighashType};
use primitives::taproot::{self, TapLeafHash};
use primitives::{
    ecdsa, transaction, Amount, FeeRate, PrivateKey, PublicKey, Transaction, TxOut, XOnlyPublicKey,
};
...
mod prelude {
    ...
    pub use primitives::hex::DisplayHex as _;
}

#[cfg(test)]
mod tests {
    use bip32::ChildNumber;
    use primitives::hashes::{hash160, ripemd160, sha256};
    use primitives::hex::{test_hex_unwrap as hex, FromHex};
    use primitives::locktime::absolute;
    use primitives::{
        secp256k1, transaction, NetworkKind, OutPoint, ScriptBuf, Sequence, TxIn, Witness,
    };
    #[cfg(feature = "rand-std")]
    ...
    #[test]
    fn combine_psbts() {
        let mut psbt1 = hex_psbt(include_str!("../../bitcoin/tests/data/psbt1.hex")).unwrap();
        let psbt2 = hex_psbt(include_str!("../../bitcoin/tests/data/psbt2.hex")).unwrap();
        let psbt_combined = hex_psbt(include_str!("../../bitcoin/tests/data/psbt2.hex")).unwrap();
    ...
    fn combine_psbts_commutative() {
        let mut psbt1 = hex_psbt(include_str!("../../bitcoin/tests/data/psbt1.hex")).unwrap();
        let mut psbt2 = hex_psbt(include_str!("../../bitcoin/tests/data/psbt2.hex")).unwrap();
    ...
    #[cfg(feature = "rand-std")]
    fn sign_psbt() {
        use primitives::{WPubkeyHash, WitnessProgram, WitnessVersion};
