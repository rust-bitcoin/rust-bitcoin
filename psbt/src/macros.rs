...
macro_rules! impl_psbt_deserialize {
    ($thing:ty) => {
        impl $crate::serialize::Deserialize for $thing {
            fn deserialize(bytes: &[u8]) -> core::result::Result<Self, $crate::Error> {
                $crate::primitives::consensus::deserialize(&bytes[..])
    ...
    ($thing:ty) => {
        impl $crate::serialize::Serialize for $thing {
            fn serialize(&self) -> $crate::prelude::Vec<u8> {
                $crate::primitives::consensus::serialize(self)
            }
        }
    };
}
...
macro_rules! impl_psbtmap_decoding {
    ($thing:ty) => {
        impl $thing {
            pub(crate) fn decode<R: $crate::primitives::io::BufRead + ?Sized>(
                r: &mut R,
