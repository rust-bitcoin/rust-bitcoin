...
impl fmt::Debug for All {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            ...
            OP_CSV => write!(f, "CSV"),
            All {code: x} if x >= OP_NOP1.code && x <= OP_NOP10.code => write!(f, "NOP{}", x - OP_NOP1.code + 1),
...
impl All {
    ...
    pub fn classify(self) -> Class {
        ...
        // 11 opcodes
                  ...
                  (OP_NOP1.code <= self.code &&
                   self.code <= OP_NOP10.code) {
            Class::NoOp
        // 75 opcodes
        } else if self == OP_RESERVED || self == OP_VER || self == OP_RETURN ||
                  self == OP_RESERVED1 || self == OP_RESERVED2 ||
                  self.code >= OP_RETURN_186.code {
            Class::ReturnOp
        // 1 opcode
        } else if self == OP_PUSHNUM_NEG1 {
            Class::PushNum(-1)
        // 16 opcodes
        } else if OP_PUSHNUM_1.code <= self.code &&
                  self.code <= OP_PUSHNUM_16.code {
            Class::PushNum(1 + self.code as i32 - OP_PUSHNUM_1.code as i32)
        // 76 opcodes
        } else if self.code <= OP_PUSHBYTES_75.code {
...
mod tests {
    ...
    macro_rules! roundtrip {
        ($unique:expr, $op:ident) => {
            assert_eq!($op, All::from($op.into_u8()));
