...
#[cfg(test)]
macro_rules! hex_script (($s:expr) => (::blockdata::script::Script::from(<Vec<u8> as ::hex::FromHex>::from_hex($s).unwrap())));

#[cfg(test)]
macro_rules! hex_hash (($s:expr) => (::util::hash::Sha256dHash::from(<[u8; 32] as ::hex::FromHex>::from_hex($s).unwrap().as_ref())));
