...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		...
		match *self {
			Error::BadWordCount(c) => write!(f, "{}: {}", desc, c),
			Error::UnknownWord(ref w) => write!(f, "{}: {}", desc, w),
			Error::BadEntropyBitCount(c) => write!(f, "{}: {} bits", desc, c),
			_ => f.write_str(desc),
		}
	}
}
...
impl error::Error for Error {
	...
	fn description(&self) -> &str {
		match *self {
			Error::BadWordCount(_) => "mnemonic has a word count that is not a multiple of 6",
			Error::UnknownWord(_) => "mnemonic contains an unknown word",
			Error::BadEntropyBitCount(_) => "entropy was not a multiple of 32 bits",
			Error::InvalidChecksum => "the mnemonic has an invalid checksum",
		}
	}
}
...
/// A mnemonic code.
///
/// Currently only the English language wordlist is supported.
...
impl str::FromStr for Mnemonic {
	...
	fn from_str(s: &str) -> Result<Mnemonic, Error> {
		...
		for (i, word) in words.iter().enumerate() {
			if let Ok(idx) = wordlist_english::WORDS.binary_search(word) {
				mnemonic.push(wordlist_english::WORDS[idx]);
...
impl Mnemonic {
	...
	pub fn from_entropy(entropy: &[u8]) -> Result<Mnemonic, Error> {
		...
		for i in 0..mlen {
			...
			words.push(wordlist_english::WORDS[idx]);
		}
		Ok(Mnemonic(words))
	}
}
...
mod test {
	...
	fn test_vectors_english() {
		...
		for vector in &test_vectors {
			...
			assert_eq!(mnemonic, &Mnemonic::from_entropy(&entropy).unwrap().to_string());
			assert_eq!(mnemonic, &Mnemonic::from_str(&mnemonic).unwrap().to_string());
			// the rest of the vector data is for seed generation
