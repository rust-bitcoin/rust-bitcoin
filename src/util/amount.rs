...
fn fmt_satoshi_in(
    ...
    let (left_pad, pad_right) = match (
        num_width < width,
        options.sign_aware_zero_pad,
        options.align.unwrap_or(fmt::Alignment::Right),
    ) {
        (false, _, _) => (0, 0),
...
impl Amount {
    ...
    ///
    /// Please be aware of the risk of using floating-point numbers.
    pub fn as_btc(self) -> f64 { self.to_float_in(Denomination::Bitcoin) }
...
impl fmt::Display for Display {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match &self.style {
            ...
            DisplayStyle::DynamicDenomination => fmt_satoshi_in(
                self.sats_abs,
                self.is_negative,
                f,
...
impl SignedAmount {
    ...
    fn to_sat_abs(self) -> u64 { self.as_sat().wrapping_abs() as u64 }
