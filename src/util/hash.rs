...
impl Sha256dHash {
  ...
  pub fn le_hex_string(&self) -> String {
    ...
    for i in range(0us, 32) {
      ret.push(from_digit((data[i] / 0x10) as usize, 16).unwrap());
      ret.push(from_digit((data[i] & 0x0f) as usize, 16).unwrap());
    }
    ret
  }
