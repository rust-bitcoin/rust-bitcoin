...
impl TaprootBuilder {
    ...
    pub fn is_finalized(&self) -> bool { self.branch.len() == 1 && self.branch[0].is_some() }
    ...
    pub fn finalize<C: secp256k1::Verification>(
        ...
        match self.branch.pop() {
            ...
            Some(Some(node)) => Ok(TaprootSpendInfo::from_node_info(secp, internal_key, node)),
...
impl NodeInfo {
    ...
    pub fn new_hidden_node(hash: sha256::Hash) -> Self {
        Self { hash: hash, leaves: vec![], has_hidden_nodes: true }
    }
...
impl fmt::Display for TaprootError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            ...
            TaprootError::InvalidParity(e) =>
                write!(f, "Invalid parity value for internal key: {}", e),
