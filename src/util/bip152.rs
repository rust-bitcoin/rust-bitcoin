...
impl fmt::Display for TxIndexOutOfRangeError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "a transaction index is requested that is \
            out of range from the corresponding block: {}", self.0,
        )
    }
}
...
mod test {
    ...
    use ::{
        Block, Transaction, TxIn, TxOut, OutPoint, Txid, BlockHash, TxMerkleNode,
        Script, BlockHeader, Witness,
    };
    ...
    fn dummy_tx(nonce: &[u8]) -> Transaction {
        Transaction {
            ...
            input: vec![TxIn {
                ...
                script_sig: Script::new(), sequence: 1, witness: Witness::new(),
            }],
