    ...
    fn fmt(&self, fmt: &mut Formatter) -> fmt::Result {
        match self.payload {
            ...
            Payload::Pubkey(ref pk) => {
                ...
                prefixed[0] = self.network.p2pk_prefix();
...
impl FromStr for Address {
    ...
    fn from_str(s: &str) -> Result<Address, encode::Error> {
        ...
        let payload_hash = Hash160::from(&data[1..]);

        let (network, payload) = BitcoinNetworks::networks_iter()
            .flat_map(|network| {
                if network.p2sh_prefix() == data[0] {
                    Some((network, Payload::ScriptHash(payload_hash)))
                } else if network.p2pkh_prefix() == data[0] {
                    Some((network, Payload::PubkeyHash(payload_hash)))
                } else {
                    None
                }
            })
            .next()
            .ok_or_else(|| encode::Error::Base58(base58::Error::InvalidVersion(vec![data[0]])))?;
...
mod tests {
    ...
    fn test_p2pk () {
        ...
        let addr = Address::p2pk(&key, Network::bitcoin());
    ...
    fn test_p2sh_parse() {
        ...
        let addr = Address::p2sh(&script, Network::bitcoin_testnet());
    ...
    fn test_p2wpkh () {
        ...
        let addr = Address::p2wpkh(&key, Network::bitcoin());
    ...
    fn test_p2wsh () {
        ...
        let addr = Address::p2wsh(&script, Network::bitcoin());
    ...
    fn test_bip173_vectors() {
        ...
        let addr = Address::from_str(addrstr).unwrap();
        assert_eq!(addr.network, Network::bitcoin_testnet());
        ...
        let addr = Address::from_str(addrstr).unwrap();
        assert_eq!(addr.network, Network::bitcoin_regtest());
