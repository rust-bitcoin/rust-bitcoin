...
#[cfg(test)]
mod tests {
    use util::misc::{to_hex_string, hex_bytes};
    use util::mnemonic::{validate, seed};
    ...
    fn test_bip39() {
        let tests = tests();
        for t in tests.iter() {
            assert_eq!(to_hex_string(&seed(t.mnemonic, "TREZOR").unwrap()), t.seed.to_string());
            assert_eq!(validate(t.mnemonic).unwrap(), hex_bytes(t.entropy).unwrap());
        }
    }

    #[test]
    fn test_invalid() {
        assert!(validate("this is not a love song").is_err());
        assert!(validate("abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon").is_err());
        assert!(validate("zoo zoo zoo zoo zoo zoo zoo zoo zoo zoo zoo zoo").is_err());
        assert!(validate("scissors invite lock maple supreme raw rapid void congress muscle digital elegant little brisk hair mango congress abandon").is_err());
    }
