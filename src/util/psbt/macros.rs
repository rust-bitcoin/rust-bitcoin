...
macro_rules! hex_psbt {
    ($s:expr) => { $crate::consensus::deserialize(&<alloc::vec::Vec<u8> as $crate::hashes::hex::FromHex>::from_hex($s).unwrap()) };
}
    ...
    ($slf:ident.$keyed_name:ident <= <$raw_key:ident: $keyed_key_type:ty>|<$raw_value:ident: $keyed_value_type:ty>) => {
        if !$raw_key.key.is_empty() {
            ...
            match $slf.$keyed_name.entry(key_val) {
                ...
                ::alloc::collections::btree_map::Entry::Occupied(_) => return Err($crate::util::psbt::Error::DuplicateKey($raw_key).into()),
            }
        } else {
            return Err($crate::util::psbt::Error::InvalidKey($raw_key).into());
        }
