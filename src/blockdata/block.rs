...
mod benches {
    ...
    const SOME_BLOCK: &'static [u8] = include_bytes!("000000000000000000000c835b2adcaedc20fdf6ee440009c249452c726dafae.raw");

    #[bench]
    pub fn bench_block_serialize(bh: &mut Bencher) {
        let raw_block = SOME_BLOCK;
        ...
        bh.iter(|| {
            let result = block.consensus_encode(&mut data).unwrap();
            black_box(&result);
            data.clear();
        });
    }
