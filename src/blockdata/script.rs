...
impl Script {
    ...
    pub fn new_v0_wpkh(pubkey_hash: &WPubkeyHash) -> Script { Script::new_v0_p2wpkh(pubkey_hash) }
    ...
    pub fn new_v0_wsh(script_hash: &WScriptHash) -> Script { Script::new_v0_p2wsh(script_hash) }
    ...
    pub fn asm(&self) -> String { Script::bytes_to_asm(self.as_ref()) }
}
...
impl Builder {
    /// Creates a new empty script.
    pub fn new() -> Self { Builder(vec![], None) }
    ...
    pub fn push_scriptint(self, data: i64) -> Builder { self.push_slice(&build_scriptint(data)) }
    ...
    pub fn into_script(self) -> Script { Script(self.0.into_boxed_slice()) }
}
...
mod test {
    ...
    fn script() {
        ...
        comp.extend(Vec::from_hex(keystr).unwrap().iter().cloned());
        assert_eq!(&script[..], &comp[..]);
    ...
    fn script_asm() {
        ...
        assert_eq!(hex_script!("01").asm(), "OP_PUSHBYTES_1 <push past end>");
        assert_eq!(hex_script!("0201").asm(), "OP_PUSHBYTES_2 <push past end>");
        assert_eq!(hex_script!("4c").asm(), "<unexpected end>");
        assert_eq!(hex_script!("4c0201").asm(), "OP_PUSHDATA1 <push past end>");
        assert_eq!(hex_script!("4d").asm(), "<unexpected end>");
        assert_eq!(hex_script!("4dffff01").asm(), "OP_PUSHDATA2 <push past end>");
