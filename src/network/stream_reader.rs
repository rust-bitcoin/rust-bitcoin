...
impl<R: Read> fmt::Debug for StreamReader<R> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result { write!(f, "StreamReader") }
}
...
impl<R: Read> StreamReader<R> {
    ...
    #[deprecated(
        since = "0.28.0",
        note = "wrap your stream into a buffered reader if necessary and use consensus_encode directly"
    )]
    pub fn new(stream: R, _buffer_size: Option<usize>) -> StreamReader<R> {
        StreamReader { stream: BufReader::new(stream) }
    }
