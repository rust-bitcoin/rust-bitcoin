...
pub use self::blockdata::{GetBlocks, GetHeaders, Inventory};
pub use self::filter::{CFilter, CFHeaders, CFCheckpt, GetCFilters, GetCFHeaders, GetCFCheckpt};
pub use self::network::{Reject, RejectReason, Version};
...
mod test {
    ...
    use {Block, BlockHeader, ServiceFlags, Transaction};
    use super::*;
    ...
    fn full_round_ser_der_raw_network_message_test() {
        ...
        let version_msg: Version = deserialize(&Vec::from_hex("721101000100000000000000e6e0845300000000010000000000000000000000000000000000ffff0000000000000100000000000000fd87d87eeb4364f22cf54dca59412db7208d47d920cffce83ee8102f5361746f7368693a302e392e39392f2c9f040001").unwrap()).unwrap();
        ...
        let msgs = vec![
            ...
            NetworkMessage::GetBlocks(GetBlocks::new(vec![hash([1u8; 32]).into(), hash([4u8; 32]).into()], hash([5u8; 32]).into())),
            NetworkMessage::GetHeaders(GetHeaders::new(vec![hash([10u8; 32]).into(), hash([40u8; 32]).into()], hash([50u8; 32]).into())),
            NetworkMessage::MemPool,
